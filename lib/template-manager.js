  ...
  templates.forEach((origTemplate) => {
    locales.forEach((locale) => {
      ...
      populateTemplateChoices(template, locale, (err) => {
        ...
        if (!templateCache[template.name][locale]) {
          templateCache[template.name][locale] = {}
        }

        templateCache[template.name][locale][template.version] = template;
      });
    });
  });
...
function getTemplate(name, version, locale) {
  ...
  if (!versions && locale !== defaultLocale) {
    return getTemplate(name, version, defaultLocale);
  }

  if (!version) {
    var versionsSortedDesc = Object.keys(versions).sort((a, b) => {
      return a < b;
    });

    version = versionsSortedDesc[0];
  }
