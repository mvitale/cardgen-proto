...
var defaultTemplateLoader = reqlib('lib/template-loader')
  ...
  , i18nFields = {
      'uiLabelKey': {
        prefix: 'template.fieldLabels',
        fieldKey: 'uiLabel'
      },
      'uiInfoKey': {
        prefix: 'template.fieldInfo',
        fieldKey: 'uiInfo'
      },
      'ctaKey': {
        prefix: 'template.fieldCtas',
        fieldKey: 'cta'
      }
    }
  ;
...
function translateTemplate(locale, template) {
  ...
  Object.keys(templateCopy.spec.fields).forEach((key) => {
    ...
    Object.keys(i18nFields).forEach((fieldKey) => {
      var fieldInfo = i18nFields[fieldKey];

      if (fieldKey in field) {
        field[fieldInfo.fieldKey] = i18n.t(locale, fieldInfo.prefix + '.' + field[fieldKey]);
      }
    });
  });
...
function getDefaultAndChoiceData(name, version, locale, params, log, cb) {
   ...
   */
  fetchData(dataSupplier, params, log, (err, results) => {
    if (err) return cb(err);
 ...
 */
function fetchData(dataSupplierName, templateParams, log, cb) {
  ...
  dataSupplier.supply(templateParams, log, cb);
}
