...
function assignDeckIdHelper(req, res, deckId) {
  ...
  Card.findOne({ userId: userId, _id: cardId }, (err, card) => {
    if (err) {
      return resUtils.errJsonRes(res, err);
    }
    ...
    if (deckId) {
      Deck.findOne({ userId: req.params.userId, _id: deckId }, (err, deck) => {
        if (err) {
          return resUtils.errJsonRes(res, err);
        }
...
function deleteCard(req, res) {
  ...
  Card.findOneAndRemove({ userId: userId, _id: cardId }, (err, card) => {
    if (err) {
      resUtils.errJsonRes(res, err);
    } else if (!card) {
      cardNotFound(res, cardId, userId);
...
function populateDeckFromCollection(req, res) {
  ...
  Deck.findOne({
    ...
    collectionCardCreator.createJob(userId, deck, colId, req.log)
      .then(function(jobId) {
        resUtils.jsonRes(res, resUtils.httpStatus.ok, {
          jobId: jobId
        });
