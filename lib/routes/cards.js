...
function cardSummaryHelper(req, res, cards) {
  ...
  for (var i = 0; i < cards.length; i++) {
    summaryPromises[i] = CardSummaryWrapper.new(cards[i])
      .catch((err) => {
        req.log.error({ err: err }, 'failed to wrap card');
      })
  }

  return Promise.all(summaryPromises)
    .then((summaries) => {
      var filteredSummaries = summaries.filter(summary => summary);
      req.log.debug({}, 'cardSummaryHelper END');
      resUtils.jsonRes(res, resUtils.httpStatus.ok, filteredSummaries);
    });
}
...
function decksForUser(req, res, next) {
  ...
  return resourceHelpers.allDecksForUser(appId, userId)
    .then((decks) => {
      return DeckWrapper.wrapAll(decks, userId);
    })
    .then((wrappedDecks) => {
      resUtils.jsonRes(
        res, 
...
function refreshMainPhotoChoices(req, res, next) {
  return resourceHelpers.cardForUser(req.appId, req.params.userId, req.params.cardId)
  .then((card) => {
    if (!card) {
      throw new VError({
        name: 'notFound'
    ...
    return card.refreshMainPhotoChoices(req.log);
  }).then((card) => {
    return card.save();
  }).then((card) => {
    return resUtils.jsonRes(res, resUtils.httpStatus.ok, new MongooseWrapper(card));
  })
  .catch(next);
}
module.exports.refreshMainPhotoChoices = refreshMainPhotoChoices;
...
function cardPng(req, res, next) {
  return Card.findOne({
    ...
    generator.generatePng(card, { widthInches: card.template().spec.width / 100 }, req.log)
    .then((png) => {
      res.writeHead(200, {
...
function deckPdfStatus(req, res, next) {
  return PdfJob.findOne({
    ...
    resUtils.jsonRes(res, resUtils.httpStatus.ok, {
      status: job.status,
      resultFileName: job.status === 'done' ? job.resultFileName() : null
    });
  }).catch(next);
}
...
function deckPdfResult(req, res, next) {
  res.sendFile(PdfJob.resultPath(req.params.fileName), {}, (err) => {
    if (err) {
      next(err);
    }
  });
...
function getPublicDecks(req, res, next) {
  return resourceHelpers.publicDecks(req.appId)
    .then((decks) => {
      return DeckWrapper.wrapAll(decks, null);
    }).then((wrapped) => {
...
function createDeckPngs(req, res, next) {
  return PngJob.create({
    deck: req.body.deckId,
    appId: req.appId,
    status: 'running'
  }).then((job) => {
...
module.exports.createDeckPngs = createDeckPngs;

function deckPngStatus(req, res, next) {
  return PngJob.findOne({
    _id: req.params.id
  }).exec().then((job) => {
    resUtils.jsonRes(res, resUtils.httpStatus.ok, {
      status: job.status,
      resultFileName: job.status === 'done' ? 
        job.resultFileName() :
        null
    });
  }).catch(next);
}
module.exports.deckPngStatus = deckPngStatus;

function deckPngResult(req, res, next) {
  res.sendFile(PngJob.resultPath(req.params.fileName), {}, (err) => {
    if (err) {
      next(err);
    }
  });
}
module.exports.deckPngResult = deckPngResult;
