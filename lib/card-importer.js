...
if (!argv.appId || !argv.file || !argv.userId) {
  console.log('Usage: $ node lib/card-importer.js --file <file_name> --userId <user_id> [--userId <user_id> ... ]  --appId <app_id>');
  process.exit(1);
}
...
Object.keys(decksByName).forEach((deckName) => {
  var rows = decksByName[deckName]
    ...
    , deckParams = {
        userId: userIds[0],
        userIds: userIds.length > 1 ? userIds.slice(1) : [],
        appId: appId, 
        name: deckName
      }
    ;
...
function makeCards(deck, rows) {
  return new Promise((resolve, reject) => {
    makeCardsHelper(deck, rows, resolve);
  });
}

function makeCardsHelper(deck, rows, resolve) {
  var row = rows.pop()
    , card = Card.new({
        appId: appId,
        userId: userIds[0],
  ...
  card.populateDefaultsAndChoices(log)
    .then((card) => {
    ...
    .then(() => {
      if (rows.length) {
        setTimeout(() => makeCardsHelper(deck, rows, resolve), 1000);
      } else {
        resolve();
      }
    });
