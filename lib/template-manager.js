...
function load() {
  ...
  templates.forEach((template) => {
    populateTemplateChoices(template, (err) => {
      if (err) throw err;
...
function populateTemplateChoices(template, cb) {
  ...
  choiceHelper(
    ...
    (err, data, tips) => {
      ...
      template.spec.choices = data;
      template.spec.choiceTips = tips;
      cb();
    }
  );
}
    ...
    return choiceHelper(
      ...
      (err, choices, tips) => {
        ...
        return defaultDataHelper(
          ...
          params,
          results,
          Object.assign({}, choices, template.spec.choices),
          Object.assign({}, tips, template.spec.choiceTips),
          {},
          (err, defaults) => {
            if (err) return cb(err);
...
function defaultDataHelper(
  ...
  supplier.supply(params, apiResults, choices[fieldId], tips[fieldId],
    (err, val, choiceIndex) => {
      ...
      console.log('blah', supplierName, choiceIndex);
