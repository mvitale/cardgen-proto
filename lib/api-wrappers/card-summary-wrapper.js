...
function CardSummaryWrapper(delegate, wrapped) {
  ...
  this.toJSON = function() {
    ...
    json.commonName = this.wrapped.isFieldNameValid('commonName') ? 
      this.wrapped.getDataAttr('commonName', 'text', '') : '';
    json.sciName = this.wrapped.isFieldNameValid('sciName') ? 
      this.wrapped.getDataAttr('sciName', 'text', '') : '';
    json.taxonGroup = this.wrapped.isFieldNameValid('taxonClass') ?
