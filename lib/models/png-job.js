...
function run(baseLogger, done) {
  ...
  .then(() => {
    var writeStream = fs.createWriteStream(this.getResultPath())
...
function getResultPath() {
  return path.join(basePath, this._id.toString() + '.zip');
}
pngJobSchema.methods.getResultPath = getResultPath;
