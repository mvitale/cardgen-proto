...
function CardSummaryWrapper(delegate, wrapped) {
  ...
  this.toJSON = function() {
    ...
    json.commonName = this.wrapped.isFieldIdValid('commonName') ? 
      this.wrapped.getDataAttr('commonName', 'text', '') : '';
    json.sciName = this.wrapped.isFieldIdValid('sciName') ? 
      this.wrapped.getDataAttr('sciName', 'text', '') : '';
    json.taxonGroup = this.wrapped.isFieldIdValid('taxonClass') ?
