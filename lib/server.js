...
/*
 * Refresh a card's photo choices and get the new JSON representation 
 */
userRouter.post('/:userId/cards/:cardId/refreshImages', cardRoutes.refreshMainPhotoChoices);
...
app.post('/deckPngs', cardRoutes.createDeckPngs);

app.get('/deckPdfs/:id/status', cardRoutes.deckPdfStatus);

app.get('/deckPdfs/downloads/:fileName', cardRoutes.deckPdfResult);

app.get('/deckPngs/:id/status', cardRoutes.deckPngStatus);

app.get('/deckPngs/downloads/:fileName', cardRoutes.deckPngResult);
...
app.get('/cachedImages/:imgUrl(*)', imageRoutes.cachedImage);
