// TODO: audit and move over to resource-helpers where appropriate.
...
function createCardInDeck(req, res) {
  ...
  resourceHelpers.deckForUser(appId, userId, deckId)
    .then((deck) => {
      if (!deck) {
...
function cardSummariesForUser(req, res) {
  resourceHelpers.allCardsForUser(req.appId, req.params.userId)
    .then((cards) => {
...
function getCard(req, res) {
  ...
  resourceHelpers.cardForUser(req.appId, userId, cardId)
    .then((card) => {
      if (!card) {
...
function decksForUser(req, res) {
  resourceHelpers.allDecksForUser(req.appId, req.params.userId)
    .then((decks) => {
      var wrappedDecks = [];
...
function setDeckDesc(req, res) {
  Deck.findOne({
    ...
    if (!deck) {
      return deckNotFound(res, req.params.deckId, userId);
    }

    deck.desc = req.body;
    saveAndSend(res, deck);
  }).catch((err) => {
    return resUtils.errJsonRes(res, err); 
  });
}
